...
# Input: directory containing VCF files from ExpansionHunter
# Output: A table where the rows are samples, the columns are regions, and the values are the EH-reported allele sizes for that region (e.g., "13/18" means that one allele has 13 repeat units and the other allele has 18 repeat units).
...
parser.add_argument("--two-column-per-locus-format", default=False, action="store_true") # Output two columns for each locus - one for the smaller allele and one for the larger allele - instead of having one column per locus formatted as described above ("13/18").
parser.add_argument("--skip-gender", default=False, action="store_true") # Skip VCF files containing "male" or "female" in the filename (unlikely to be of use)
...
for vcf_filename in Path(args.base_dir).rglob('*.vcf'):
    if args.skip_gender and "male" in str(vcf_filename): # Skip VCF files containing "male" or "female" in the filename
        continue
